#!/usr/bin/perl
{
	...
	sub output_top {
		...
		print $cgi->h2("Sequence Processing");
	}
	...
	sub display_results {
		...
		print $cgi->img({-src=>"data:image/png;base64,$encoded_str",-align=>"center"});
	...
	sub blast_sequence {
		...
		$req = new HTTP::Request GET => "http://www.ncbi.nlm.nih.gov/blast/Blast.cgi?CMD=Get&FORMAT_TYPE=Text&RID=25A6RM6D014";
		$response = $ua->request($req);
	...
	sub get_fasta {
		...
		my(@acc_array) = @_;
		$file = "tmpfastaoutput.fa";
		open( FASTA, ">$file" ) || die "cannot open the file - $!\n";
